...
> **Generated:** 2026-02-05T10:23:49.977806
...
Poetry configuration with dependencies and tool settings - Configure project metadata, Python version constraint (>=3.11,<3.13), core dependencies (fastapi, uvicorn, sqlalchemy, alembic, redis, pydantic, python-jose, passlib, bcrypt), development dependencies (pytest, black, ruff, mypy, pre-commit), and tool configurations for black, ruff, pytest, mypy in single file
...
- **rule_guide:** # Yaml Config

# Complexity Assessment Guidelines

## Simple YAML Tasks (< 30 lines, basic config)
...
- Minimal use of lists
- No anchors or references
- Simple string values
- Avoid multi-line strings unless necessary
- No complex conditionals

**Example structure for simple task:**
```yaml
...
## Medium Complexity (30-100 lines, structured config)
...
- Logical grouping with nesting
- YAML anchors for DRY
- Multi-line strings for scripts
- Lists for repeated items
- Environment-specific overrides
- Basic templating if supported
- Comments for documentation

## Complex YAML Tasks (> 100 lines, system config)
...
- Custom tags if supported
- Extensive documentation

# CRITICAL RULES:

1. **Indentation is critical** - Always use spaces, never tabs; be consistent (2 or 4 spaces)

2. **Quotes when needed** - Quote strings that look like numbers, booleans, or have special characters

3. **Avoid Norway problem** - Quote country codes (NO becomes false)

4. **Multi-line strings matter** - Use | for literal, > for folded, know the difference

5. **Type coercion awareness** - YAML auto-detects types; be explicit when needed

## Configuration File Types and Patterns:
...
**Complex (multiple resources with templating):**
```yaml
---
apiVersion: v1
kind: ConfigMap
...
**For Simple Configs, DON'T:**
- Use complex anchors for 2 values
- Create deep nesting beyond 3 levels
- Use multi-document YAML
- Add templating for static values
- Over-comment obvious settings

**For Complex Configs, DON'T:**
- Repeat values (use anchors)
- Skip schema validation
- Ignore documentation
- Mix concerns in one file
- Use ambiguous type coercion
...
# Binary data
image: !!binary |
  R0lGODlhAQABAIAAAAAAAA
```

## Tool-Specific Patterns:

### CloudFormation/AWS
- Use !Ref and !GetAtt functions
- Separate parameters, resources, outputs
- Use mappings for region-specific values

### Azure ARM Templates
- Though JSON, often authored as YAML
- Use parameters for flexibility
- Implement linked templates

### Terraform (HCL, but often configured via YAML)
...
- **related_files_content:** ["README.md", ".gitignore"]
...
Create the file at `pyproject.toml` implementing the changes described above.
